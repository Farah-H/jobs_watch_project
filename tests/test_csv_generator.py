...
from definitions import TEST_RESOURCES_FOLDER, SEPERATOR, OUTPUT_LOC 
import pytest
import csv
import os
...
class TestCSVGenerator:
    ...
    def create_top_30_test_csv(self, generate_top_30_html_object):
        ...
        os.remove(f'{TEST_RESOURCES_FOLDER}top_30_without_headers.csv')
    ...
    def test_csv_without_headers(self):
        list_count = 0
        with open(f'{TEST_RESOURCES_FOLDER}top_30_without_headers.csv') as top_30_without_headers:
    ...
    def test_csv_with_headers(self):
        list_count = 0
        with open(f'{TEST_RESOURCES_FOLDER}{SEPERATOR}top_30_with_headers.csv') as top_30_without_headers:
            parsed_csv_file = csv.reader(top_30_without_headers)
            for line in parsed_csv_file:
                if len(line) != 0:
                    list_count += 1
    ...
    def test_csv_without_headers_default_location(self):
        list_count = 0
        with open(f'{OUTPUT_LOC}{SEPERATOR}ItJobsWatchTop30.csv') as top_30_without_headers:
            parsed_csv_file = csv.reader(top_30_without_headers)
            for line in parsed_csv_file:
                if len(line) != 0:
                    list_count += 1
